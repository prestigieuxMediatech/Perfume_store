...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const res = await axios.get(`${BASE}/api/admin/blogs/${id}`, { headers: getHeaders() });
        const blog = res.data;
        setForm({
          title: blog.title || "",
          slug: blog.slug || "",
          category_label: blog.category_label || "Journal",
          excerpt: blog.excerpt || "",
          content: blog.content || "",
          author_name: blog.author_name || "7EVEN Editorial",
          read_time_minutes: String(blog.read_time_minutes || 4),
          is_published: Boolean(blog.is_published),
          existing_cover_image: blog.cover_image || "",
        });
        setPreview(blog.cover_image ? `${BASE}${blog.cover_image}` : "");
      } catch {
  ...
  const removeImage = () => {
    setImage(null);
    setPreview("");
    setForm((prev) => ({ ...prev, existing_cover_image: "" }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${BASE}/api/admin/blogs/${id}`, formData, {
        headers: {
          ...getHeaders(),
  ...
  if (loading) {
    return <p style={{ color: "#7A7264", fontSize: "0.72rem", letterSpacing: "0.1em" }}>Loading blog...</p>;
  }
    ...
    <div style={{ maxWidth: "820px" }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <textarea className="ad-input" rows={14} name="content" value={form.content} onChange={handleChange} style={{ resize: "vertical", lineHeight: "1.8" }} />
        </div>
